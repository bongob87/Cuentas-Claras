...
import { formatCurrency, APP_LOGO } from '../utils';
import { 
  Users, 
  TrendingUp, 
  TrendingDown, 
...
export const Reports: React.FC<ReportsProps> = ({ customers, user }) => {
  ...
  const handleExportPDF = () => {
    ...
    // --- Header ---
    if (APP_LOGO.startsWith('data:image')) {
       // Simple validation to ensure it's base64
       try {
           doc.addImage(APP_LOGO, 'PNG', 14, 10, 15, 15);
       } catch (e) {
    ...
    doc.setFontSize(20);
    doc.text("Reporte Financiero", 35, 18);
    
    doc.setFontSize(10);
    doc.text(`Generado: ${today}`, 35, 24);
    if (user?.storeName) {
        doc.text(`Tienda: ${user.storeName}`, 35, 29);
    }
    ...
    // Left Column
    doc.text("Total por Cobrar:", 20, yPos + 18);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0);
    doc.text(formatCurrency(summaryData.totalOwed), 60, yPos + 18);
    
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text("Total Vencido:", 20, yPos + 26);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(220, 38, 38); // Red
    doc.text(formatCurrency(summaryData.totalOverdue), 60, yPos + 26);

    // Right Column
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text("Clientes Activos:", 110, yPos + 18);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0);
    doc.text(summaryData.activeClients.toString(), 150, yPos + 18);

    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text("Fiado este mes:", 110, yPos + 26);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(37, 99, 235); // Blue
    doc.text(formatCurrency(creditsThisMonth), 150, yPos + 26);
    ...
    autoTable(doc, {
        startY: yPos + 5,
        head: [['Fecha', 'Cliente', 'Tipo', 'Detalle', 'Monto']],
        body: tableBody,
        theme: 'grid',
    ...
    // Save
    doc.save(`Reporte_CuentasClaras_${format(new Date(), "yyyyMMdd")}.pdf`);
  };
  ...
  return (
    <div className="animate-in fade-in duration-300 pb-safe space-y-6 relative">
       ...
       <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <button 
                ...
                className="text-xs font-bold text-white bg-slate-900 dark:bg-slate-700 px-3 py-2 rounded-xl flex items-center gap-1.5 shadow-sm active:scale-95 transition-transform"
                title="Descargar PDF"
            >
                <Download size={16} />
                PDF
            ...
            <button 
                ...
                className="text-xs font-bold text-primary-600 bg-primary-50 dark:bg-primary-900/20 px-3 py-2 rounded-xl flex items-center gap-1.5 active:scale-95 transition-transform"
            >
                <FileText size={16} />
                Bitácora
            </button>
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* Total Owed */}
          <div className="bg-slate-900 dark:bg-slate-800 p-5 rounded-3xl border border-slate-800 shadow-sm relative overflow-hidden text-white">
             <div className="absolute -right-6 -bottom-6 opacity-10 text-white">
                <BarChart3 size={120} />
             </div>
             <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Total por Cobrar</p>
             <h3 className="text-3xl font-bold">{formatCurrency(summaryData.totalOwed)}</h3>
