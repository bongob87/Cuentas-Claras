...
import { X, TrendingUp, TrendingDown, CheckCircle, Search, User, AlertTriangle, Loader2 } from 'lucide-react';
...
interface QuickTransactionModalProps {
  ...
  onClose: () => void;
  customers: Customer[];
  onConfirm: (customerId: string, transaction: Omit<Transaction, 'id'>) => Promise<void>;
}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
                <button 
                    type="submit"
                    disabled={loading}
                    className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${type === 'CREDIT' ? 'bg-primary-600 shadow-primary-600/30' : 'bg-emerald-600 shadow-emerald-600/30'} disabled:opacity-70 disabled:cursor-not-allowed`}
                >
                    {loading ? (
