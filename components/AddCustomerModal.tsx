...
import { X, User, Phone, MapPin, Camera, Loader2 } from 'lucide-react';
import { Customer } from '../types';

interface AddCustomerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (customer: Omit<Customer, 'id' | 'createdAt' | 'transactions' | 'avatarUrl'> & { avatarUrl?: string }) => Promise<void>;
}
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-dark-card w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom duration-300">
        <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={onClose} disabled={loading} className="p-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 disabled:opacity-50">
                <X size={20} />
            </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-center mb-6">
                <div className="relative">
                    ...
                    <label className={`absolute bottom-0 right-0 p-2 bg-primary-600 rounded-full text-white shadow-lg cursor-pointer hover:bg-primary-700 transition-colors ${loading ? 'pointer-events-none opacity-50' : ''}`}>
                        <Camera size={16} />
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center gap-3">
              ...
              <div className="flex-1">
                  ...
                  <input required type="text" className="w-full bg-transparent border-none outline-none text-slate-900 dark:text-white font-medium" placeholder="Ej. Juan Pérez" value={name} onChange={e => setName(e.target.value)} disabled={loading} />
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center gap-3">
              ...
              <div className="flex-1">
                  ...
                  <input required type="tel" className="w-full bg-transparent border-none outline-none text-slate-900 dark:text-white font-medium" placeholder="Ej. 55 1234 5678" value={phone} onChange={e => setPhone(e.target.value)} disabled={loading} />
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center gap-3">
              ...
              <div className="flex-1">
                  ...
                  <input type="text" className="w-full bg-transparent border-none outline-none text-slate-900 dark:text-white font-medium" placeholder="Ej. Av. Principal #123" value={address} onChange={e => setAddress(e.target.value)} disabled={loading} />
              </div>
            </div>
          ...
          <div className="mt-8 pt-4">
            <button 
                type="submit" 
                disabled={loading}
                className="w-full py-4 rounded-xl bg-primary-600 text-white hover:bg-primary-700 font-bold shadow-lg shadow-primary-600/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
