...
import { Search, Plus, ArrowLeft, CheckCircle, AlertTriangle, ChevronRight, Wallet, Calendar, History, ArrowUpRight, ArrowDownLeft, Users } from 'lucide-react';
  ...
  return (
    <div className="animate-in fade-in duration-300 relative h-full flex flex-col pb-safe">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 shrink-0">
          <div className="bg-slate-900 dark:bg-slate-800 p-4 rounded-2xl border border-slate-800 shadow-sm flex flex-col justify-center relative overflow-hidden text-white ring-1 ring-white/10">
              <div className="absolute -right-4 -bottom-4 opacity-10">
                  <Users size={64} />
              </div>
              <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider mb-0.5 relative z-10">Total por Cobrar</span>
              <span className="text-xl font-bold truncate relative z-10">{formatCurrency(totalReceivable)}</span>
          </div>
          <div className="bg-white dark:bg-dark-card p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center relative overflow-hidden">
              <span className="text-[10px] font-bold text-red-400 uppercase tracking-wider mb-0.5">Vencido (+30 días)</span>
              <span className="text-xl font-bold text-red-500 truncate">{formatCurrency(totalOverdue)}</span>
          </div>
      </div>
...
const CustomerDetail: React.FC<{ 
  ...
  // Dynamic Badge Logic
  let statusConfig = { label: 'Sin Deuda', color: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400', icon: CheckCircle };
  if (total > 0) {
      if (overdue >= total * 0.5) {
          statusConfig = { label: 'Alto Riesgo', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: AlertTriangle };
      } else if (overdue > 0) {
          statusConfig = { label: 'Riesgo Medio', color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', icon: AlertTriangle };
      } else {
          statusConfig = { label: 'Confiable', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', icon: CheckCircle };
      }
  }
        ...
        <div className="bg-white dark:bg-dark-card rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 text-center relative overflow-hidden">
             ...
             <div className="relative z-10">
                ...
                <div className="flex justify-center items-center gap-2 mt-2">
                    <span className={`${statusConfig.color} text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 transition-colors duration-300`}>
                        <statusConfig.icon size={10} /> {statusConfig.label.toUpperCase()}
                    </span>
                </div>
             </div>
