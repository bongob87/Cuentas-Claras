...
import { LogOut, Copy, Check, Users, Store, ArrowRight, Loader2, Bell, Smartphone, DollarSign, Calendar, RefreshCw, Moon, Sun, User as UserIcon, Shield } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ user, onLogout, isDarkMode, toggleTheme }) => {
  ...
  return (
    <div className="animate-in fade-in duration-300 pb-safe space-y-6">
       ...
       {/* Profile Card */}
       <div className="bg-slate-900 dark:bg-slate-800 rounded-3xl p-6 shadow-xl ring-1 ring-white/10 text-white relative overflow-hidden flex items-center gap-4">
            <div className="absolute -right-6 -bottom-10 opacity-10">
                <UserIcon size={140} />
            </div>
            <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center shrink-0 backdrop-blur-sm relative z-10">
               {user.photoURL ? (
            ...
            <div className="min-w-0 relative z-10">
                <h3 className="font-bold text-lg truncate">{user.displayName || 'Usuario'}</h3>
       ...
       <div className="bg-white dark:bg-dark-card rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
           <div className="flex items-center gap-2 mb-4">
               {isDarkMode ? <Moon size={20} className="text-primary-600 dark:text-primary-400" /> : <Sun size={20} className="text-primary-600 dark:text-primary-400" />}
               <h3 className="font-bold text-lg text-slate-900 dark:text-white">Apariencia</h3>
           </div>
           ...
           <div className="space-y-4">
               <div className="flex items-center justify-between">
                   <div>
                       ...
                       <p className="text-xs text-slate-500">Cambiar tema de la aplicación</p>
                   </div>
                   <button 
                       onClick={toggleTheme}
                       className={`w-12 h-7 rounded-full transition-colors relative ${isDarkMode ? 'bg-primary-600' : 'bg-slate-200 dark:bg-slate-700'}`}
                   >
                       <div className={`w-5 h-5 bg-white rounded-full shadow-sm absolute top-1 transition-transform ${isDarkMode ? 'left-6' : 'left-1'}`}></div>
                   </button>
               </div>
           </div>
       </div>
