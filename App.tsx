...
import { generateId, APP_LOGO } from './utils';
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
      if (typeof window !== 'undefined') {
          const savedTheme = localStorage.getItem('cuentas_claras_theme');
  ...
  // 2. Theme Effect (Global Application)
  useEffect(() => {
      if (isDarkMode) {
          document.documentElement.classList.add('dark');
          localStorage.setItem('cuentas_claras_theme', 'dark');
      } else {
          document.documentElement.classList.remove('dark');
          localStorage.setItem('cuentas_claras_theme', 'light');
      }
  }, [isDarkMode]);
  ...
  // 4. Data Subscription & Background Checks
  useEffect(() => {
    if (!user) {
        setCustomers([]);
        return;
  ...
  // 5. Offline/Online Status Listeners
  useEffect(() => {
      const handleOffline = () => {
  ...
  if (!user) {
      return <AuthScreen isDarkMode={isDarkMode} toggleTheme={toggleTheme} />;
  }
